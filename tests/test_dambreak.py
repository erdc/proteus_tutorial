...
def test_run_dambreak_serial():
    ...
    runCommand = "cd "+currentPath+"/../2d/; parun --TwoPhaseFlow dambreak.py -l 5 -C \"final_time=0.2\" -D ../tests/serial"
...
def test_check_for_failure_serial():
    ...
    if (text.find('Step failed') == -1) and (text.find('Done with garbage') != -1):
        ...
        subprocess.check_call("rm "+currentPath+"/serial/dambreak*",shell=True)
    else:
...
def test_run_dambreak_parallel():
    """Test restart workflow"""
    currentPath = os.path.dirname(os.path.abspath(__file__))
    runCommand = "cd "+currentPath+"/../2d/; mpiexec -np 2 parun --TwoPhaseFlow dambreak.py -l 5 -C \"final_time=0.2\" -D ../tests/parallel"
    subprocess.check_call(runCommand,shell=True)
...
def test_check_for_failure_parallel():
    ...
    if (text.find('Step failed') == -1) and (text.find('Done with garbage') != -1):
        ...
        subprocess.check_call("rm "+currentPath+"/parallel/dambreak*",shell=True)
    else:
