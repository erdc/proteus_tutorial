...
def test_run_flat_plate_serial():
    ...
    runCommand = "cd "+currentPath+"/../2d/; parun --TwoPhaseFlow flat_plate.py -l 5 -C \"final_time=0.2\" -D ../tests/serial"
...
def test_check_for_failure_serial():
    ...
    if (text.find('Step failed') == -1) and (text.find('Done with garbage') != -1):
        ...
        subprocess.check_call("rm -r serial/flat_plate*",shell=True)
    else:
...
def test_run_flat_plate_parallel():
    """Test restart workflow"""
    currentPath = os.path.dirname(os.path.abspath(__file__))
    runCommand = "cd "+currentPath+"/../2d/; mpiexec -np 2 parun --TwoPhaseFlow flat_plate.py -l 5 -C \"final_time=0.2\" -D ../tests/parallel"
    subprocess.check_call(runCommand,shell=True)
...
def test_check_for_failure_parallel():
    ...
    if (text.find('Step failed') == -1) and (text.find('Done with garbage') != -1):
        ...
        subprocess.check_call("rm -r parallel/flat_plate*",shell=True)
    else:
